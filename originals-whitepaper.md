...
2. Key Rotation & Recovery: Before an asset is inscribed, authority can be handed on by a signed rotateKey event, which replaces the authorized key set outright — retired keys are dead from that event forward. After inscription the creator’s key lineage is frozen: no rotation is permitted at the did:btco layer. Holding the satoshi confers the right to append — entries signed by the holder’s own key and witnessed by a reinscription that postdates the anchor — but no control over the creator’s key set, and holder entries cannot make authenticity claims about the work. A creator who loses a key after inscription therefore cannot rotate away from it, and there is no revocation. This follows from ownership being satoshi control rather than a key the protocol can reassign: if holding the satoshi could reassign the key set, the satoshi holder could rewrite authorship, which is exactly what the separation exists to prevent.
