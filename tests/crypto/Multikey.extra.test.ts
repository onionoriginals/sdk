...
describe('Multikey encode/decode', () => {
  ...
  test('decode errors: invalid base58 character', () => {
    expect(() => multikey.decodePublicKey('z0')).toThrow('Invalid base58 character');
  });
  ...
  test('decode errors: unsupported type header', () => {
    ...
    const mb = 'z' + Buffer.from(concatBytes(unknownHeader, fakeKey)).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, '');
