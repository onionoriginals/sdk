...
- KTD4. **Deposit UTXO reads move off the free public API to QuickNode.** Replace the mempool.space address API with a QuickNode read. The address-to-UTXO surface is unproven: `QuickNodeProvider` speaks Bitcoin Core RPC plus the ordinals methods, and Core has no address index. U4 verifies the endpoint's method list before building; if the add-on cannot serve address UTXOs, stop and re-decide between a Blockbook-style add-on, a self-hosted index, and a paid mempool.space tier. *(session-settled: user-directed — chosen over a mempool.space enterprise sponsorship or self-hosting: QuickNode is already paid for, and the free public API is not sanctioned for server-side multi-user polling.)* Governs R4.
