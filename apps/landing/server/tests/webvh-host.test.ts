...
describe('webvh-host store', () => {
  ...
  test('served content is neutralized against stored XSS', async () => {
    ...
    const key = 'victim.example.com/published/anonymous/evil/did.jsonl';
    await store.handlePut(
      putReq(key, '<script>alert(document.cookie)</script>', 'text/html'),
      new URL(`http://host/api/host/${encodeURIComponent(key)}`)
    );
    const url = new URL('http://victim.example.com/published/anonymous/evil/did.jsonl');
  ...
  test('size cap: body over maxObjectBytes is rejected 413', async () => {
    ...
    const key = 'd/published/anonymous/x/did.jsonl';
  ...
  test('rate limit keys on the passed socket IP, not a spoofable X-Forwarded-For', async () => {
    ...
    const first = await store.handlePut(mk('a/published/anonymous/x/did.jsonl', '9.9.9.9'), u('a/published/anonymous/x/did.jsonl'), '1.1.1.1');
    expect(first.status).toBe(200);
    // Same real IP, a DIFFERENT spoofed X-Forwarded-For → still the same bucket → limited.
    const second = await store.handlePut(mk('b/published/anonymous/x/did.jsonl', '8.8.8.8'), u('b/published/anonymous/x/did.jsonl'), '1.1.1.1');
    expect(second.status).toBe(429);
    // A genuinely different socket IP gets its own bucket.
    const other = await store.handlePut(mk('c/published/anonymous/x/did.jsonl', '7.7.7.7'), u('c/published/anonymous/x/did.jsonl'), '2.2.2.2');
...
describe('webvh-host eviction', () => {
  test('a client at its own cap evicts its own LRU group and accepts the write', async () => {
    ...
    await put(store, 'd/published/anonymous/a1/did.jsonl');
    await put(store, 'd/published/anonymous/a1/cel.json');
    await put(store, 'd/published/anonymous/a2/did.jsonl');
    await put(store, 'd/published/anonymous/a2/cel.json');

    const res = await put(store, 'd/published/anonymous/a3/did.jsonl');
    expect(res.status).toBe(200);
    expect(hit(store, 'd/published/anonymous/a3/did.jsonl')).toBe(true);
    // The oldest publish went, whole.
    expect(hit(store, 'd/published/anonymous/a1/did.jsonl')).toBe(false);
    expect(hit(store, 'd/published/anonymous/a1/cel.json')).toBe(false);
    expect(hit(store, 'd/published/anonymous/a2/did.jsonl')).toBe(true);
  });
  ...
  test('a full store accepts the next write instead of answering 507', async () => {
    ...
    expect((await put(store, 'a/published/anonymous/x/did.jsonl')).status).toBe(200);
    expect((await put(store, 'b/published/anonymous/x/did.jsonl')).status).toBe(200);
  });
  ...
  test('one client flooding never evicts another client’s published log', async () => {
    ...
    await put(store, 'victim.test/published/anonymous/v/did.jsonl', '2.2.2.2', 'the victim log');
    for (let i = 0; i < 30; i++) {
      await put(store, `flood.test/published/anonymous/f${i}/did.jsonl`, '1.1.1.1');
    }
    const url = new URL('http://victim.test/published/anonymous/v/did.jsonl');
    const served = store.serve(new Request(url), url);
    expect(served).not.toBeNull();
  ...
  test('an evicted entry reads as a clean miss, not an error', async () => {
    const store = createWebvhHostStore({ maxEntriesPerClient: 1 });
    await put(store, 'd/published/anonymous/g1/did.jsonl');
    await put(store, 'd/published/anonymous/g2/did.jsonl');

    const res = store.read(u('d/published/anonymous/g1/did.jsonl'));
    expect(res.status).toBe(404);
    expect(await res.json()).toEqual({ error: 'not_found' });
    const url = new URL('http://d/published/anonymous/g1/did.jsonl');
    expect(store.serve(new Request(url), url)).toBeNull();
  });
  ...
  test('a recently READ group is not the one evicted', async () => {
    ...
    await put(store, 'd/published/anonymous/g1/did.jsonl');
    await put(store, 'd/published/anonymous/g2/did.jsonl');
    expect(hit(store, 'd/published/anonymous/g1/did.jsonl')).toBe(true); // g1 is now the most recent

    await put(store, 'd/published/anonymous/g3/did.jsonl');
    expect(hit(store, 'd/published/anonymous/g1/did.jsonl')).toBe(true);
    expect(hit(store, 'd/published/anonymous/g2/did.jsonl')).toBe(false);
  });
  ...
  test('eviction is all-or-nothing: a surviving log never points at gone bytes', async () => {
    ...
    const groupA = ['d/published/anonymous/u/a/did.jsonl', 'd/published/anonymous/u/a/cel.json', 'd/published/anonymous/u/a/resources/zAbC'];
    for (const k of groupA) await put(store, k);
    await put(store, 'd/published/anonymous/u/b/did.jsonl');
  ...
  test('a single publish bigger than the cap takes itself down, not a neighbour', async () => {
    const store = createWebvhHostStore({ maxEntriesPerClient: 2 });
    await put(store, 'other.test/published/anonymous/o/did.jsonl', '2.2.2.2');
    // One group, four objects, one client: the group restarts rather than
    // leaving a log whose resources are gone.
    await put(store, 'd/published/anonymous/u/a/did.jsonl');
    await put(store, 'd/published/anonymous/u/a/cel.json');
    await put(store, 'd/published/anonymous/u/a/resources/z1');
    await put(store, 'd/published/anonymous/u/a/resources/z2');
    expect(hit(store, 'other.test/published/anonymous/o/did.jsonl')).toBe(true);
    expect(hit(store, 'd/published/anonymous/u/a/resources/z2')).toBe(true);
  });
  ...
  test('entries still expire on the TTL, independently of eviction', async () => {
    ...
    expect(hit(store, 'd/published/anonymous/g1/did.jsonl')).toBe(true);
    clock += 501;
    expect(hit(store, 'd/published/anonymous/g1/did.jsonl')).toBe(false);
  ...
  test('the global entry cap bounds a distributed flood without a 507', async () => {
    ...
    for (let i = 0; i < 12; i++) {
      const res = await put(store, `d/published/anonymous/g${i}/did.jsonl`, `10.0.0.${i}`);
      expect(res.status).toBe(200);
    }
    expect(store.stats().entries).toBeLessThanOrEqual(4);
    expect(hit(store, 'd/published/anonymous/g11/did.jsonl')).toBe(true); // the newest writer survives
  });

  test('the per-client bookkeeping map is bounded too', async () => {
    const store = createWebvhHostStore({ maxClients: 3 });
    for (let i = 0; i < 12; i++) await put(store, `d/published/anonymous/g${i}/did.jsonl`, `10.0.0.${i}`);
  ...
  test('a total-bytes budget bounds memory regardless of entry count', async () => {
    ...
    for (let i = 0; i < 12; i++) await put(store, `d/published/anonymous/g${i}/did.jsonl`, '1.1.1.1', '0123456789');
...
describe('webvh-host cross-identity publish groups', () => {
  const alice = ['d/published/anonymous/u/alice/did.jsonl', 'd/published/anonymous/u/alice/resources/r1'];

  test('a revision from a second IP does not split the publish across two budgets', async () => {
    const store = createWebvhHostStore({ maxEntriesPerClient: 2 });
    await put(store, 'd/published/anonymous/u/alice/did.jsonl', 'A');
    await put(store, 'd/published/anonymous/u/alice/resources/r1', 'A');
    // Same visitor, rotated egress address, revising her own log.
    await put(store, 'd/published/anonymous/u/alice/did.jsonl', 'B');

    // …then the ORIGINAL identity fills its budget.
    const written = [...alice, 'd/published/anonymous/u/other/did.jsonl', 'd/published/anonymous/u/other/cel.json'];
    await put(store, 'd/published/anonymous/u/other/did.jsonl', 'A');
    await put(store, 'd/published/anonymous/u/other/cel.json', 'A');

    expect(orphanedResources(store, written)).toEqual([]);
    // Whichever way it went, the publish shared one fate.
    expect(hit(store, 'd/published/anonymous/u/alice/resources/r1')).toBe(hit(store, 'd/published/anonymous/u/alice/did.jsonl'));
  });
  ...
  test('…and the same holds when the SECOND identity is the one that floods', async () => {
    ...
    const written = [...alice, 'd/published/anonymous/u/bob/did.jsonl', 'd/published/anonymous/u/bob/cel.json'];
    await put(store, 'd/published/anonymous/u/bob/did.jsonl', 'B');
    await put(store, 'd/published/anonymous/u/bob/cel.json', 'B');
  ...
  test('no eviction sequence leaves a resolvable log pointing at gone bytes', async () => {
    ...
    for (const [p, ip] of ips.entries()) {
      const members = [
        `d/published/anonymous/u/p${p}/did.jsonl`,
        `d/published/anonymous/u/p${p}/cel.json`,
        `d/published/anonymous/u/p${p}/resources/r${p}a`,
        `d/published/anonymous/u/p${p}/resources/r${p}b`,
      ];
      ...
      await put(store, `d/published/anonymous/u/p${p}/did.jsonl`, 'R');
    }
    // Every original identity then keeps working until its own budget evicts.
    for (const ip of ips) {
      for (let j = 0; j < 5; j++) await put(store, `d/published/anonymous/u/${ip}extra${j}/did.jsonl`, ip);
    }
