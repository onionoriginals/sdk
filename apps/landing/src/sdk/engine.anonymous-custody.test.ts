/**
 * #598 — publication must not silently strand authoring capability, and a
 * "local backup" that lets anyone who can read this browser's storage
 * decrypt it is not a real confidentiality boundary.
 *
 ...
 * write, wrapped under this browser's own non-extractable key (`keystore.ts`)
 * rather than a passphrase stored next to its own ciphertext; a same-browser
 * reload restores it transparently; and a browser with no local storage or
 * durable key store is refused up front rather than losing the key silently.
 */
...
describe('anonymous authorship custody survives a reload (#598)', () => {
  ...
  test('a browser with no local storage is refused before any hosting write', async () => {
    ...
    try {
      ...
      await expect(engine.publish()).rejects.toThrow(/local storage|key store/i);
  ...
  test('a browser with no durable key store is refused before any hosting write', async () => {
    host = installCel3Host();
  ...
  test('a transparent local backup whose decrypted key does not match its recorded controller is refused, not silently used', async () => {
    ...
    stored.controller = 'did:key:zNotTheRealController';
...
describe('the transparent local backup this browser writes on publish', () => {
  ...
  test('is keyed by asset id and never stores the raw secret key bytes or a passphrase', async () => {
    host = installCel3Host();
    const engine = new DemoEngine();
    const state = await engine.create('Original', 'Upload', 'bytes');
    ...
    expect(parsed.ciphertextBase64).toBeTruthy();
    // Unlike a passphrase-colocated backup, this record carries no secret
    // that alone would let a reader of `storage` decrypt it — the wrapping
    // key lives only as a non-extractable CryptoKey in the browser's key
    // store (see the next test), never as JSON in this record.
    expect(JSON.stringify(parsed)).not.toContain('"secretKey"');
    expect(JSON.stringify(parsed)).not.toContain('"passphrase"');
  });

  test('the wrapping key is non-extractable: reading storage alone cannot recover it', async () => {
    host = installCel3Host();
    const engine = new DemoEngine();
    await engine.create('Original', 'Upload', 'bytes');
    await engine.publish();
