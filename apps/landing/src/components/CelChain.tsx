...
/**
 * A migrate event names its destination as `layer` ('webvh' | 'btco') plus the
 * full `targetDid` — not a `to` field. Colour by that destination so the chain
 * reads in the same palette as the pipeline above it.
 */
function accentFor(entry: CelEntry): string {
  if (entry.type === 'migrate') {
    const layer = str(entry.data.layer) ?? str(entry.data.targetDid) ?? '';
...
function summarize(entry: CelEntry): string {
  ...
  if (entry.type === 'migrate') {
    const target = str(data.targetDid);
  ...
  if (entry.type === 'updateResource') return 'Records a new resource version';
  return entry.type;
}
