...
- Post-anchor `rotateKey`, `deactivate`, and `migrate` are rejected outright; a v1 `transfer` (`data.newController`) is rejected anywhere. The v0 legacy `transfer` shape stays readable only AT OR BEFORE the anchor — post-anchor it is rejected like every other non-update type, because a v0 transfer signed by the creator's frozen key after selling the sat is indistinguishable from a fresh forgery. Off-chain post-anchor appends — including the witness-acknowledgment updates earlier SDK versions wrote — no longer verify.
