...
- **Required on mainnet, named at boot.** `validateConfig` reports an unset or malformed `VITE_WEBVH_HOST` when `BTC_NETWORK=mainnet`, alongside the existing contract; a value carrying a scheme, port or path is rejected, since the DID embeds it verbatim. Warn-only until `CONFIG_STRICT=1`, like every other rule.
