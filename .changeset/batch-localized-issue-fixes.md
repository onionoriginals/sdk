...
**BREAKING — CEL `digestMultibase` is now a spec-conformant multihash (#258):** `computeDigestMultibase` prepends the sha2-256 multihash header (`0x12 0x20`); `decodeDigestMultibase` validates/strips it and rejects legacy header-less values. `previousEvent` chain links and external-reference digests produced by older releases use the bare-digest format and will not match — existing CEL logs must be recomputed.
