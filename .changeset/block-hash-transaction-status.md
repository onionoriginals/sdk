...
Add optional `blockHash` to `OrdinalsProvider.getTransactionStatus()`, returned by `QuickNodeProvider` (already available from verbose `getrawtransaction`, previously discarded after resolving height) and `SignetProvider` (from ord's `/tx/<txid>` response).
