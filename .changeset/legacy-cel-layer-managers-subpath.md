...
**`PeerCelManager`, `WebVHCelManager`, and `BtcoCelManager`, and their config/data types, are no longer exported from the `@originals/cel` package root** (#597).
...
- **Removed from the root, moved to `@originals/cel/legacy`:** `PeerCelManager`, `WebVHCelManager`, `BtcoCelManager`, and the types `PeerCelConfig`, `CelAssetData`, `PeerAssetData`, `WebVHCelConfig`, `WebVHMigrationData`, `BtcoCelConfig`, `BtcoMigrationData`. Import all of these from `@originals/cel/legacy` instead of `@originals/cel`.
- `CelSigner`, `CelLayer`, `OriginalsCel`, `OriginalsCelConfig`, and `OriginalsCelOptions` are unaffected and remain at the package root.
