...
export class RegtestProvider implements OrdinalsProvider {
  ...
  async getSatSnapshot(satoshi: string): Promise<SatSnapshot> {
    return readSatSnapshot({
      ...
      status: () => this.ord('/status'),
      indexHash: async height => {
        const bytes = await this.bytes(`${this.ordUrl}/blockhash/${height}`);
        return bytes === null ? null : new TextDecoder().decode(bytes).trim();
      },
      sat: sat => this.ord(`/sat/${sat}`),
      inscription: id => this.ord(`/inscription/${id}`),
      content: id => this.bytes(`${this.ordUrl}/content/${id}`),
      metadata: id => this.ord(`/r/metadata/${id}`),
    }, satoshi, 'regtest');
  }
