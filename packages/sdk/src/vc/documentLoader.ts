...
export class DocumentLoader {
  ...
  private async resolveDID(didUrl: string): Promise<LoadedDocument> {
    ...
    if (!didDoc) {
      ...
      // registerVerificationMethod — but ONLY for the self-certifying method
      // (did:key) whose key material is bound into the identifier and which
      // has no hosted, revocable authoritative state. For hosted methods
      ...
      // and key rotation published by the authoritative source. (did:peer
      // support is removed entirely — legacy did:peer credentials no longer
      // verify; there is no read path.)
    ...
    if (fragment) {
      ...
      // verification method, and ONLY for the self-certifying method
      // (did:key) whose key material is bound into the identifier. For hosted
