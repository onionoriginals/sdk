        ...
        for (const status of entries) {
          ...
          try {
            ...
            const trust = await validateStatusListCredentialTrust(
              ...
              matchingList,
              async (listVC) => {
                // Signature-only: the status list credential does not carry
                // its own credentialStatus, so there is nothing recursive to
                // check, and using verifyCredential's resolver-based status
