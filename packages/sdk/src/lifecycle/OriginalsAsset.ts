      ...
      for (const res of this.resources) {
        ...
        if (res.content !== undefined) {
          if (!(res.content instanceof Uint8Array)) {
            return verificationFailure(
              'RESOURCE_INVALID',
              `Resource "${res.id}" has inline content that is not a byte array.`,
              { resourceId: res.id }
            );
          }
          const data = res.content;
          if (res.size !== undefined && res.size !== data.byteLength) {
            return verificationFailure(
              'RESOURCE_INVALID',
              `Resource "${res.id}" declares ${res.size} bytes but contains ${data.byteLength}.`,
              { resourceId: res.id }
            );
          }
