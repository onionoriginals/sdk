...
test("an accepted resolution and its DID metadata are labeled provider-asserted unless the adapter itself claims independent chain validation", async () => {
  ...
  expect(validatedMetadata.didDocumentMetadata.chainEvidence).toBe("node-validated");
  expect(validatedMetadata.didDocumentMetadata.source).toBe("regtest-core+ord");
});
