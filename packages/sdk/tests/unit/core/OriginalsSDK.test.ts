...
describe('OriginalsSDK', () => {
  ...
  describe('createOriginal', () => {
    test('routes to createDIDOriginal for type "did"', async () => {
      const mockSigner = {
        sign: mock(() => Promise.resolve({ proofValue: 'test-signature' })),
        getVerificationMethodId: () => 'did:webvh:example.com#key-1',
      };
      ...
      // The actual call will use the real createDID from didwebvh-ts
      const result = await OriginalsSDK.createOriginal(options);
  ...
  describe('createDIDOriginal', () => {
    test('creates DID original with required parameters', async () => {
      const mockSigner = {
        sign: mock(() => Promise.resolve({ proofValue: 'test-signature' })),
        getVerificationMethodId: () => 'did:webvh:example.com#key-1',
      };
      ...
      const result = await OriginalsSDK.createDIDOriginal(options);
      expect(result).toBeDefined();
      expect(result.did).toBeDefined();
    ...
    test('includes custom context when provided', async () => {
      const mockSigner = {
        sign: mock(() => Promise.resolve({ proofValue: 'test-signature' })),
        getVerificationMethodId: () => 'did:webvh:example.com#key-1',
      };
      ...
      const options = {
        ...
        updateKeys: ['key1'],
        verificationMethods: [],
        context: ['https://www.w3.org/ns/did/v1', 'https://custom.context'],
      };
  ...
  describe('updateOriginal', () => {
    test('routes to updateDIDOriginal for type "did"', async () => {
      const mockSigner = {
        sign: mock(() => Promise.resolve({ proofValue: 'test-signature' })),
        getVerificationMethodId: () => 'did:webvh:example.com#key-1',
      };
      ...
      const created = await OriginalsSDK.createOriginal(createOptions);
      ...
      const result = await OriginalsSDK.updateOriginal(updateOptions);
      expect(result).toBeDefined();
      expect(result.did).toBeDefined();
      expect(result.log.length).toBeGreaterThan(created.log.length);
    });
  ...
  describe('updateDIDOriginal', () => {
    test('updates DID original with required parameters', async () => {
      const mockSigner = {
        sign: mock(() => Promise.resolve({ proofValue: 'test-signature' })),
        getVerificationMethodId: () => 'did:webvh:example.com#key-1',
      };
      ...
      const created = await OriginalsSDK.createDIDOriginal(createOptions);
      ...
      const result = await OriginalsSDK.updateDIDOriginal(updateOptions);
      expect(result).toBeDefined();
      expect(result.did).toBeDefined();
      expect(result.doc).toBeDefined();
      expect(result.log).toBeDefined();
      expect(result.log.length).toBe(created.log.length + 1);
      expect(result.meta).toBeDefined();
    });
    ...
    test('includes optional parameters when provided', async () => {
      ...
      const updateOptions = {
        ...
        verifier: mockVerifier,
        updateKeys: ['key2'],
        controller: 'did:webvh:controller.com',
        alsoKnownAs: ['did:webvh:alias.com'],
        portable: true,
      };
