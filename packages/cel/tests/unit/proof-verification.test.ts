...
import type { DataIntegrityProof, CreateOptions, UpdateOptions, EventLog } from '../../src/types';
...
describe('CEL Proof Verification', () => {
  ...
  test('6. Non-Ed25519 did:key (ES256K): eddsa-jcs-2022 + secp256k1 VM → verified: false', async () => {
    ...
    const { generateTestKeyPair } = await import('../fixtures/celSigner');
    const secp256k1Pair = generateTestKeyPair('ES256K');
