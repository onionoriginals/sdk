...
import { createDidManagerKeyResolver } from '../../src/keyResolver';
...
import type { CelDidResolver } from '../../src/keyResolver';
...
const buildWebvhLog = async (): Promise<EventLog> => {
  ...
  const webvhMgr = new WebVHCelManager(createMockSigner(), 'coverage.example.com', [], { acknowledgeNonConformantIdentifier: true });
