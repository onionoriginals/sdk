...
import { canonicalizeEvent, canonicalizeEntryForChain, witnessSigningBytes } from '../../src/canonicalize';
import { computeDigestMultibase, decodeDigestMultibase } from '../../src/hash';
...
describe('verifyEventLog', () => {
  ...
  describe('deactivation is terminal (issue #257)', () => {
    async function makeDeactivatedLogWithTrailingUpdate(): Promise<EventLog> {
      const { signer, verificationMethod: vm } = await makeRealSigner();
      const { deactivateEventLog } = await import('../../src/algorithms/deactivateEventLog');
      const { computeDigestMultibase } = await import('../../src/hash');
      const { canonicalizeEntryForChain } = await import('../../src/canonicalize');
...
describe('issue #240: honest WitnessService attestations verify', () => {
  test('witnessEvent + verifyEventLog round-trip reports the witness proof as verified', async () => {
    const { witnessEvent } = await import('../../src/algorithms/witnessEvent');
    const ed25519 = await import('@noble/ed25519');
