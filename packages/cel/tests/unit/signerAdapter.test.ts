...
import { celSignerFromKeyPair, createKeyStoreCelSigner, hexSha256ToDigestMultibase } from '../../src/signerAdapter';
import { generateTestKeyPair } from '../fixtures/celSigner';
...
describe('CEL signer adapter', () => {
  test('keypair variant produces a log verifyEventLog accepts as did:cel', async () => {
    ...
    const kp = generateTestKeyPair('Ed25519');
  ...
  test('rejects non-Ed25519 keys', async () => {
    ...
    const kp = generateTestKeyPair('ES256K');
