...
const createWebvhLog = async (): Promise<EventLog> => {
  ...
  const webvhManager = new WebVHCelManager(createMockSigner(), 'example.com', [], { acknowledgeNonConformantIdentifier: true });
...
describe('BtcoCelManager', () => {
  ...
  describe('integration: peer to webvh to btco migration', () => {
    it('should complete full migration cycle', async () => {
      ...
      // Migrate to webvh
      const webvhManager = new WebVHCelManager(createMockSigner(), 'gallery.example.com', [], { acknowledgeNonConformantIdentifier: true });
