...
const web = 'did:webvh:' + base58.encode(Buffer.from(digest(Buffer.from('syntax-valid alias fixture')).slice(1), 'base64url')) + ':example.com:art'; // Valid multihash syntax only; separate WebVH method binding is unverified.
...
const web2 = 'did:webvh:' + base58.encode(Buffer.from(digest(Buffer.from('second syntax-valid alias fixture')).slice(1), 'base64url')) + ':example.com:other';
...
writeFileSync(new URL('histories.json', here), JSON.stringify({
  scope: 'Hand-worked decision histories with real signatures from public test keys. Positions, completeness, ownership and tips are DECLARED SCENARIOS, not Bitcoin observations. WebVH alias multihashes are syntactically valid; method-log binding is not verified. This companion preserves all 42 original hand-worked decisions and changes only the alias inputs and dependent signed bytes/digests. The original decision fixtures remain frozen. This is not production or regtest evidence.',
