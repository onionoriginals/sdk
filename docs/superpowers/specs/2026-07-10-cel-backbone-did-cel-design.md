...
- **Phase 2 — convergence.** LifecycleManager writes CEL events at every operation (signer adapter); `OriginalsAsset` views derived from the log; `verify()` → `verifyEventLog`; migrate events wired to the Phase-0 real layer minting.
- **Phase 3 — interchange + hand-off.** `serialize()`/`loadAsset()` envelope; verify-on-load; creator→buyer round-trip integration test (create → publish → inscribe → serialize → load in fresh process → verify → resolve every historical DID forward to head).
