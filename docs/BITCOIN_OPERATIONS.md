...
- **Authorship ops append CEL events:** `createAsset` (`create`), `publishToWeb`
  ...
  `addResourceVersion` (`update`), and holder/creator statements
  (`appendStatement` → `update`). Key rotation does not exist on did:btco:
  `rotateKey` is rejected after the anchor and `rotateBtcoKeys` always throws.
