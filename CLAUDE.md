...
- `transferOwnership()` - A pure Bitcoin **sat move** — writes NOTHING to the CEL (ownership IS sat control; the sat move hands over the RIGHT TO APPEND). Ownership is read live via `getCurrentOwner()`, never from a log event. The `transfer` CEL event type: v0 legacy shape (`previousOwner`/`newOwner`) is read-only accepted PRE-anchor only (post-anchor it is rejected like every other non-update type); a v1 shape (`data.newController`) is rejected anywhere.
